...
VLQAna::VLQAna(const edm::ParameterSet& iConfig) :
  ...
  t_eventExampleVar       (consumes<vector<double>> (iConfig.getParameter<edm::InputTag>("eventExampleVar"))),
  ...
  eventExampleVarMin_     (iConfig.getParameter<double>           ("eventExampleVarMin")), //retrieve cut value from cfi file
...
bool VLQAna::filter(edm::Event& evt, const edm::EventSetup& iSetup) {
  ...
  Handle<vector<double>> h_eventExampleVar     ; evt.getByToken(t_eventExampleVar     , h_eventExampleVar     ) ; //assuming eventExampleVar is a B2GNTuple variable
  ...
  double eventExampleVar(*h_eventExampleVar.product()); //Retrieve value of variable from NTuple
  ...
  if (eventExampleVar < eventExampleVarMin_) return false; //applying the cut
