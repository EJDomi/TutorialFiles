  ...
  jetID_(JetIDParams_, iC) 
{
  ...
  if ( type_ == AK8JET ) {
    ...
    t_jetExampleVar     = iC.consumes<vector<float>>(JetSubstrParams_.getParameter<edm::InputTag> ("jetExampleVarLabel") ); //this assumes variable is a jet substructure parameter of AK8 jet
  ...
  if ( type_ == AK8JET ) {
    ...
    idxjetExampleVarMin_  = iConfig.getParameter<double>("jetExampleVarMin") ;
    idxjetExampleVarMax_  = iConfig.getParameter<double>("jetExampleVarMax") ;
...
void JetMaker::operator()(edm::Event& evt, vlq::JetCollection& jets) {
  ...
  for (unsigned ijet = 0; ijet < (h_jetPt.product())->size(); ++ijet) { 
    ...
    if ( type_ == AK8JET ) {
      ...
      Handle <vector<float>>  h_jetExampleVar   ; evt.getByToken (t_jetExampleVar     , h_jetExampleVar); //This is for getting variable from B2GNTuple, only to be used if
                                                                                                          //variable is stored in the Events tree of a B2GNTuple
      ...
      jetExampleVar   = (h_jetExampleVar.product())->at(ijet) ; //Pulls the value of the variable from the jet at index 'ijet'
      ...
      if ( abs(jettau2/jettau1) >= idxjettau2Bytau1Min_ 
          ...
          && jetExampleVar >= idxjetExampleVarMin_  //where the actual cut takes place
          && jetExampleVar < idxjetExampleVarMax_
